      ...
      <div className="mt-10 space-y-10">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
            <p>
              <strong className="text-blue-900">Email:</strong> {teacher?.persnalDetails?.email}
            </p>
            <p>
              <strong className="text-blue-900">Gender:</strong> {teacher?.persnalDetails?.gender}
            </p>
            <p>
              <strong className="text-blue-900">Country:</strong> {teacher?.persnalDetails?.country}
            </p>
            <p>
              <strong className="text-blue-900">Languages:</strong>{" "}
        ...
        <section>
          ...
          <div className="space-y-2 text-gray-700">
            ...
            <p>
              <strong className="text-blue-900">Bio:</strong> {teacher?.profesnalDetails?.bio}
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {teacher?.profesnalDetails?.specializations?.map((spec, idx) => (
              <span
                key={idx}
                className="bg-blue-50 text-blue-800 px-3 py-1 rounded-full text-sm shadow-sm"
              >
                {spec}
              </span>
            ))}
        ...
        {/* Courses */}
        <section>
          <h2 className="text-xl font-semibold text-blue-900 mb-4">Courses</h2>
          <div className="space-y-4">
            {teacher?.profesnalDetails?.taught?.length ? (
              teacher.profesnalDetails.taught.map((c, idx) => (
                ...
                >
                  <p>
                    <strong className="text-blue-900">Course Name:</strong> {c.subject}
                  </p>
                  <p>
                    <strong className="text-blue-900">Student:</strong> {c.studentName} ({c.studentDetail?.country})
                  </p>
                  <p>
                    <strong className="text-blue-900">Total class:</strong> {c.numberOfClass}
                  </p>
                  <p>
                    <strong className="text-blue-900">Duration:</strong> {c.durationInDays}
                  </p>
                  <p>
                    <strong className="text-blue-900">lastClass:</strong> {c.endDate}
                  </p>
                </div>
              ))
            ) : (
