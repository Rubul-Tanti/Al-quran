  ...
  useEffect(() => {
    if (data?.data) {
      const initialData = {
        ...
        perHourRate: data?.data?.perHourRate || '',
        startingDate:now,
  ...
  const createClass = async () => {
    try {
      const response = await createClassApi({
        ...
        perHourRate: finalProposal.perHourRate,
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {page === "editDetails" ? (
                ...
                <div className="bg-white border flex-1 border-gray-200 rounded-lg p-4">
                  ...
                  <p className="text-2xl font-bold text-green-400">
                    <input
                      id="perHourRate"
                      onChange={handleChange}
                      value={Proposal.perHourRate}
                      className="max-w-[55px] border border-green-200 outline-0 rounded-lg p-1"
                      type="number"
                    />{" "}
          ...
          <div className="bg-gray-50 border-2 border-gray-200 rounded-lg p-6 space-y-4">
            ...
            <div className="flex flex-row gap-2">
              <p>Per Hour Rate:</p>
              <p className="text-sm text-zinc-600">${finalProposal.perHourRate}</p>
            </div>
